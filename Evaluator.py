...
class EvaluatorDetection(object):
    def __init__(self, path_to_lmdb_dir):
        transform = transforms.Compose([
            transforms.CenterCrop([54, 54]),
            transforms.ToTensor(),
        ...
        self._loader = torch.utils.data.DataLoader(DataSetDetection(path_to_lmdb_dir, transform), batch_size=128, shuffle=False)
    ...
    def evaluate(self, model):
        ...
        with torch.no_grad():
            for batch_idx, (images, isdigit) in enumerate(self._loader):
                images, isdigit= images.cuda(), isdigit.cuda()
                isdigit_logits = model.eval()(images)

                isdigit_pred = isdigit_logits.max(1)[1]

                num_correct += isdigit_pred.eq(isdigit.long()).cpu().sum()
